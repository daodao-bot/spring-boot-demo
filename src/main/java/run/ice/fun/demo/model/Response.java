...
import run.ice.fun.demo.error.ErrorEnum;
...
public class Response<D> implements Serializer {
    ...
    @JsonIgnore
    public Boolean isOk() {
        return Boolean.valueOf(AppError.OK.code.equals(this.code)); // && null != this.data
    }

    public static <D> Boolean isOk(Response<D> response) {
        return Boolean.valueOf(null != response && response.isOk());
    }
